...
def compute_dense_reward(self, action, obs) -> float:
    ...
    goal_pos = self.goal_position # This is the target 3D position for the EE to press the button
    ...
    # `self.robot.gripper_openness` ranges from -1 (fully closed) to 1 (fully open).
    # We penalize deviations from the fully closed state (-1.0).
    gripper_target_openness = -1.0
    reward_gripper_closed = -W_GRIPPER_CLOSED * abs(self.robot.gripper_openness - gripper_target_openness)
